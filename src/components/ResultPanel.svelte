  ...
  {:else}
    ...
    {#if result?.response}
      <div class="mt-4">
        ...
        <pre
          class="text-xs font-mono bg-white/50 p-2 rounded overflow-x-auto whitespace-pre-wrap break-all">{JSON.stringify(
            result.response,
            null,
            2
          )}</pre>
    ...
    {#if result?.decoded}
      <div class="mt-4">
        ...
        <pre
          class="text-xs font-mono bg-white/50 p-2 rounded overflow-x-auto whitespace-pre-wrap break-all">{result.decoded}</pre>
      </div>
    {/if}
    {#if result?.formatted}
      <div class="mt-4">
        ...
        <pre
          class="text-xs font-mono bg-white/50 p-2 rounded overflow-x-auto whitespace-pre-wrap break-all">{result.formatted}</pre>
      </div>
    {/if}
  {/if}
</div>
